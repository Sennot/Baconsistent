...
inline std::optional<std::size_t> markerIndexForSegment(int segmentIndex, std::size_t markerCount) {
    // Segment 0 is the special run from 0% to the first StartPos and therefore
    // has no StartPos object of its own. Segment 1 starts at marker 0, etc.
    if (segmentIndex <= 0) {
        return std::nullopt;
    }

    auto const markerIndex = static_cast<std::size_t>(segmentIndex - 1);
