...
bool TrainingPlan::decrement(std::size_t index) {
    if (index >= m_counts.size() || m_counts[index] <= 0) {
        return false;
    }
    --m_counts[index];
    return true;
...
bool TrainingPlan::complete(std::size_t index) {
    if (index >= m_counts.size()) {
        return false;
    }
    auto const goal = target(index);
    if (m_counts[index] >= goal) {
        return false;
    }
    m_counts[index] = goal;
    return true;
