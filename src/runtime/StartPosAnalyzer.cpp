...
StartPosAnalysis analyzeStartPositions(PlayLayer* layer) {
    ...
    // Keep the live StartPos objects normalized in the SAME physical order as
    // the persistent boundary pair. This makes stage -> StartPos an exact index
    // mapping instead of a nearest-percentage lookup (which could select the
    // wrong/highest marker around duplicates or rounding).
    constexpr double epsilon = 0.01;
    double lastLegacy = 0.0;
    double lastModern = 0.0;
    for (auto const& marker : markers) {
        if (marker.legacy21 <= epsilon || marker.modern22 <= epsilon ||
            marker.legacy21 >= 100.0 - epsilon || marker.modern22 >= 100.0 - epsilon) {
            continue;
        }
        if (marker.legacy21 - lastLegacy <= epsilon ||
            marker.modern22 - lastModern <= epsilon) {
            continue;
        }
        ...
        lastLegacy = marker.legacy21;
        lastModern = marker.modern22;
    }
    return result;
}
