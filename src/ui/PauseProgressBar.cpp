...
bool updatePauseProgressBar(PauseLayer* layer) {
    ...
    value->setColor(colors.muted);

    auto const rangeText = manager.segmentRangeText(index);
    std::string titleText;
    if (theme::isTurkmenistan()) {
        titleText = fmt::format("TURKMEN CONSISTENCY  {}", rangeText);
    }
    else if (theme::isUdmurtia()) {
        titleText = fmt::format("UDMURT CONSISTENCY  {}", rangeText);
    }
    else if (theme::isTatarstan()) {
        titleText = fmt::format("TATAR CONSISTENCY  {}", rangeText);
    }
    else {
        titleText = fmt::format("BACONSISTENT  {}", rangeText);
    }
    title->setString(titleText.c_str());
    ...
    if (manager.attemptBlockedByPractice() || manager.attemptBlockedByNoclip()) {
        value->setColor(colors.danger);
        fill->setColor(colors.danger);
    }
    else {
        fill->setColor(left == 0 ? colors.success : colors.accent);
    }
    return true;
}
