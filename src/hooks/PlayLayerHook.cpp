...
class $modify(BaconsistentPlayLayer, PlayLayer) {
    ...
    bool init(GJGameLevel* level, bool useReplay, bool dontCreateObjects) {
        ...
        if (level && !level->isPlatformer()) {
            ...
            m_fields->profileActive = manager.hasActiveProfile();
            m_fields->profileId = manager.levelKey();
            m_fields->layoutRevision = manager.layoutRevision();
    ...
    void destroyPlayer(PlayerObject* player, GameObject* object) {
        ...
        auto& manager = baconsistent::TrainingManager::get();
        if (!manager.hasActiveProfile() || !wasAlive || !player || m_levelEndAnimationStarted) {
            return;
        }
    ...
    void postUpdate(float dt) {
        ...
        // Binding and unbinding happens from PauseLayer. Detect a profile
        // transition on resume so no pre-bind gameplay/noclip detector state can
        // leak into the newly selected profile attempt.
        auto const profileActive = manager.hasActiveProfile();
        auto const profileId = manager.levelKey();
        if (profileActive != m_fields->profileActive || profileId != m_fields->profileId) {
            m_fields->profileActive = profileActive;
            m_fields->profileId = profileId;
            m_fields->layoutRevision = manager.layoutRevision();
            m_fields->noclipDetector.reset();
        ...
        // The experimental in-level Stage Editor can change profile boundaries
        // while PauseLayer is open. Resume with a fresh attempt so an old
        // A->B target is never mixed with the newly edited layout.
        if (m_fields->layoutRevision != manager.layoutRevision()) {
            m_fields->layoutRevision = manager.layoutRevision();
            m_fields->lastMode = manager.percentageMode();
            manager.beginAttempt(
                baconsistent::currentPercentForMode(this, m_fields->lastMode),
