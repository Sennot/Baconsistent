    ...
    const CCSize &cellSize)
{
    ...
    for (int delta : {-1, 1})
    {
        ...
        auto button = CCMenuItemSpriteExtra::create(sprite, this,
            menu_selector(StageRangeCell::onAdjustPasses));
        button->setTag(delta);
        button->setID(delta < 0 ? "remove-pass" : "add-pass");
...
void StageRangeCell::updateStatusBadge()
{
    ...
    const auto text = fmt::format("{}/{}", m_range->completionCounter, m_requiredPasses);
    m_statusLabel->setVisible(true);
    m_statusLabel->setString(text.c_str());
    m_statusLabel->limitLabelWidth(104.f, .24f, .14f);
    m_statusLabel->setColor(m_checked ? ccColor3B{120, 255, 105} : ccColor3B{255, 220, 90});
}
...
{
    // Keep the original checkbox as a shortcut: complete / reopen this part.
    const int delta = m_checked ? m_requiredPasses - 1 - m_range->completionCounter
                                : m_requiredPasses - m_range->completionCounter;
    applyPassAdjustment(delta);
    // CCMenuItemToggler flips its state once more after invoking the callback.
    m_checkbox->toggle(m_checked);
}
...
void StageRangeCell::onAdjustPasses(CCObject *sender)
{
    if (auto button = typeinfo_cast<CCMenuItemSpriteExtra *>(sender))
        applyPassAdjustment(button->getTag());
}

void StageRangeCell::applyPassAdjustment(int delta)
{
    if (m_disabled || !m_level || m_id.empty())
        return;
