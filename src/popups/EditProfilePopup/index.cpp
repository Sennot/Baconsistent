...
{
  if (!Popup::init(320, 302, "GJ_square01_custom.png"_spr))
    return false;
  ...
  auto passesLabel = CCLabelBMFont::create("Passes per part", "bigFont.fnt");
  passesLabel->setScale(.4f);
  passesLabel->setAnchorPoint({0.f, .5f});
  passesLabel->setPosition({12.f, 62.f});
  m_mainLayer->addChild(passesLabel);
  m_passesInput = TextInput::create(74.f, "20", "bigFont.fnt");
  m_passesInput->setFilter("0123456789");
  m_passesInput->setMaxCharCount(4);
  m_passesInput->setScale(.8f);
  m_passesInput->setPosition({273.f, 62.f});
  m_passesInput->setString(std::to_string(profile->requiredPasses));
  m_mainLayer->addChild(m_passesInput);
...
void EditProfilePopup::onSave(CCObject *)
{
  ...
  const int requiredPasses = utils::numFromString<int>(m_passesInput->getString()).unwrapOr(0);
  if (requiredPasses < 1 || requiredPasses > bacon::maxPasses)
  {
    FLAlertLayer::create("Passes per part", "Enter a number from 1 to 9999.", "OK")->show();
    return;
  }
  ...
  if (m_percentagesList->hasChanged())
  {
    geode::createQuickPopup(
        ...
        "Cancel",
        "Confirm",
        [this, newStartposes, requiredPasses](auto, bool confirmed)
        {
          if (!confirmed)
            return;
          ...
          merged.requiredPasses = requiredPasses;
          bacon::refreshCompletion(merged);
