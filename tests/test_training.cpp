...
int main() {
    ...
    {
        // v0.5.3 legacy recovery only recognizes pre-profile persistence
        // identities. Modern profile-* payloads must never appear as legacy.
        auto online = parseLegacySavedKey("levels.online:123456.counts");
        assert(online.has_value());
        assert(online->levelKey == "online:123456");
        assert(online->suffix == "counts");

        auto local = parseLegacySavedKey("levels.local:0123abcd.startpos-2-1");
        assert(local.has_value());
        assert(local->levelKey == "local:0123abcd");
        assert(local->suffix == "startpos-2-1");

        assert(!parseLegacySavedKey("levels.profile-deadbeef.counts").has_value());
        assert(!parseLegacySavedKey("profiles.online:123456.name").has_value());
        assert(legacySessionFallbackName("online:42") == "Legacy Online #42");
        assert(legacySessionFallbackName("local:abcdef012345").find("abcdef01") != std::string::npos);
    }
    ...
    {
        // v0.4.4 regression: stage -> physical StartPos is exact and has no
        // off-by-one at either end. The 0% stage has no StartPos object; the
        // final stage uses the final physical StartPos, not a fallback marker.
        assert(!markerIndexForSegment(0, 9).has_value());
        assert(markerIndexForSegment(1, 9).value() == 0);
        assert(markerIndexForSegment(9, 9).value() == 8);
        assert(!markerIndexForSegment(10, 9).has_value());
    }
    ...
    {
        // v0.5.6 manual progress: +1 / -1 / complete only change repetition
        // counters and remain clamped to [0, target].
        TrainingPlan plan(4, 15);
        assert(plan.increment(3));
        assert(plan.count(3) == 1);
        assert(plan.decrement(3));
        assert(plan.count(3) == 0);
        assert(!plan.decrement(3));
        assert(plan.complete(3));
        assert(plan.count(3) == 15);
        assert(plan.completed(3));
        assert(!plan.complete(3));
        assert(!plan.increment(3));
        assert(plan.decrement(3));
        assert(plan.count(3) == 14);
        assert(!plan.completed(3));
    }
    ...
    {
        // v0.6.0 experimental layout editor: only inner boundaries move, the
        // paired 2.1/2.2 ordering stays valid and reset restores the profile.
        StageLayoutDraft draft(
            {0.0, 20.0, 50.0, 80.0, 100.0},
            {0.0, 18.0, 47.0, 82.0, 100.0}
        );
        assert(draft.valid());
        assert(draft.markerCount() == 3);
        assert(draft.moveMarker(1, 55.0, 53.0));
        assert(close(draft.legacy21()[2], 55.0));
        assert(close(draft.modern22()[2], 53.0));
        assert(draft.changed());

        // A dragged line cannot cross its neighbours.
        assert(draft.moveMarker(1, 99.0, 99.0));
        assert(draft.legacy21()[2] < draft.legacy21()[3]);
        assert(draft.modern22()[2] < draft.modern22()[3]);

        draft.reset();
        assert(!draft.changed());
        assert(close(draft.legacy21()[2], 50.0));
        assert(close(draft.modern22()[2], 47.0));
    }
